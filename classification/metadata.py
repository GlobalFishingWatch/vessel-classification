...
FishingRange = namedtuple("FishingRange", ["start_time", "end_time", "is_fishing"])
...
class VesselMetadata(object):
    def __init__(self, metadata_dict, fishing_ranges_map):
        ...
        logging.info("Vessels with both types of data: %d", len(intersection_ids))
    ...
    def ids_for_split(self, split):
        ...
        if set(self.metadata_by_split[TRAINING_SPLIT].keys()) & set(
            self.metadata_by_split[TEST_SPLIT].keys()
        ):
            logging.warning("id in both training and test split")
        return self.metadata_by_split[split].keys()

    def weighted_training_list(
        self,
        random_state,
        split,
        ...
        if missing:
            replicated_ids = np.concatenate(
                [replicated_ids, np.random.choice(replicated_ids, missing)]
            )
    ...
    def fishing_range_only_list(self, random_state, split):
        replicated_ids = []
        fishing_id_set = set([k for (k, v) in self.fishing_ranges_map.items() if v])
        fishing_range_only_ids = [
            id_ for id_ in self.ids_for_split(split) if id_ in fishing_id_set
        ]
        logging.info(
            "Fishing range training ids: %d / %d",
...
def read_vessel_time_weighted_metadata_lines(
    available_ids, lines, fishing_range_dict, split
):
    ...
    for row in lines:
        ...
        if id_ in available_ids:
            ...
            for rng in fishing_range_dict[id_]:
                time_for_this_id += (rng.end_time - rng.start_time).total_seconds()
...
def read_vessel_time_weighted_metadata(
    available_ids, metadata_file, fishing_range_dict={}, split=0
):
    reader = metadata_file_reader(metadata_file)

    return read_vessel_time_weighted_metadata_lines(
        available_ids, reader, fishing_range_dict, split
    )


def read_vessel_multiclass_metadata_lines(available_ids, lines, fishing_range_dict):
    ...
    for split, counts in dataset_kind_counts.items():
        ...
        for atomic_vessel_type, count in counts.items():
            dataset_kind_weights[split][atomic_vessel_type] = np.sqrt(
...
def read_vessel_multiclass_metadata(
    available_ids, metadata_file, fishing_range_dict={}
):
    reader = metadata_file_reader(metadata_file)
