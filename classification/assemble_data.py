...
if __name__ == '__main__':
    ...
    parser.add_argument('--fishing-range-dir', required=True, help='source directory for time range files')
    # classification/data/combined_fishing_ranges.csv'
    parser.add_argument('--fishing-range-output-path', required=True, help='location to save generated file to')
    parser.add_argument('--training-class-input-path', default=None, help='path to training classes to augment')
    parser.add_argument('--training-class-output-path', default=None, help='output path for augmented training classes')
    parser.add_argument('--force', action='store_true')
    args = parser.parse_args()

    if os.path.exists(args.fishing_range_output_path) and not args.force:
        print('fishing ranges already exist, exiting (use `--force` to overwrite)')
        raise SystemExit()

    assemble_fishing_data(args.fishing_range_dir, args.fishing_range_output_path)
    ...
    if args.training_class_output_path is None:
        ...
        if os.path.exists(args.training_class_output_path) and not args.force:
            print('classes already exist, exiting (use `--force` to overwrite)')
            raise SystemExit()
        augment_training_list(args.training_class_input_path, args.training_class_output_path, args.fishing_range_output_path)
