...
def ydump_metrics(doc, results):
    ...
    def clean(x):
        x = x.replace('_', ' ')
        x = x.split(' or ')
        x = '/'.join([y.capitalize() for y in x])
        return x

    cm = confusion_matrix(consolidated)

    missing = np.alltrue(cm.raw == 0, axis=0) & np.alltrue(cm.raw == 0, axis=1) 
    cm = ConfusionMatrix(cm.raw[~missing][:, ~missing], cm.scaled[~missing][:, ~missing]) 

    labels = [clean(x) for (x, m) in zip(results.label_list, missing) if not m]

    with tag('div', klass='unbreakable'):
        line('h3', 'Confusion Matrix')
        ydump_confusion_matrix(doc, cm, labels)
