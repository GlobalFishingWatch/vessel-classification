...
class VesselMetadataFileReaderTest(tf.test.TestCase):
    ...
    fishing_range_dict = {
        b"100001": [
            metadata.FishingRange(datetime(2015, 3, 1), datetime(2015, 3, 2), 1.0)
        ],
        b"100002": [
            metadata.FishingRange(datetime(2015, 3, 1), datetime(2015, 3, 2), 1.0)
        ],
    ...
    def test_metadata_file_reader(self):
        ...
        available_vessels = set(
            six.ensure_binary(str(x)) for x in range(100001, 100014)
        )
    ...
    def test_fixed_time_reader(self):
        ...
        result = metadata.read_vessel_time_weighted_metadata_lines(
            available_vessels, parsed_lines, self.fishing_range_dict, "Test"
        )
    ...
    def _check_splits(self, result):
        ...
        self.assertEqual(
            result.metadata_by_split["Test"][b"100001"][0],
            {
        ...
        self.assertEqual(
            result.metadata_by_split["Test"][b"100005"][0],
            {
                "label": "trawlers",
                "length": "10.0",
                "id": "100005",
        ...
        self.assertEqual(
            result.metadata_by_split["Training"][b"100002"][0],
            {
                "label": "drifting_longlines",
                "length": "24.0",
                "id": "100002",
        ...
        self.assertEqual(
            result.metadata_by_split["Training"][b"100003"][0],
            {
                "label": "drifting_longlines",
                "length": "7.0",
                "id": "100003",
        ...
        for coarse, fine_list in metadata.VESSEL_CATEGORIES:
            for fine in fine_list:
                if fine not in names:
                    names.append(fine)
        self.assertEqual(sorted(names), sorted(metadata.VESSEL_CLASS_DETAILED_NAMES))
