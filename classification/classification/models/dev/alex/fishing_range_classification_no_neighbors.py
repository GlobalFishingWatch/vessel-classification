...
class Model(abstract_models.MisconceptionWithFishingRangesModel):
    ...
    def build_training_net(self, features, timestamps, mmsis):
        # Take only the first 11 features, omitting the numNeighbors feature
        # to make feature generation easier
        features = tf.slice(features, [0, 0, 0, 0], [-1, -1, -1, 11])

        features = self.zero_pad_features(features, extra_padding=1)
