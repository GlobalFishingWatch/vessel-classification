...
def dropout_layer(inputs, is_training, keep_prob=0.5, name='dropout-layer'):
    with tf.variable_scope(name):
        kp = tf.cond(is_training, lambda: tf.constant(keep_prob), lambda: tf.constant(1.0))
...
def conv1d_layer(inputs, filter_size, filter_count, stride=1, 
                 padding="SAME", name='conv1d-layer'):
    with tf.variable_scope(name):
        h, w, n = [int(x) for x in inputs.get_shape().dims[1:]]
        assert w == 1
        W = weight_variable([filter_size, 1, n, filter_count])
        b = bias_variable([filter_count])
        return (tf.nn.conv2d(inputs, W, 
                     strides=[1, stride, 1, 1], padding=padding) + b)


def batch_norm(inputs, is_training, alpha=0.5, test_decay=0.999, epsilon=1e-3, name='batch-norm'):
    """
    """
    n = int(inputs.get_shape().dims[-1])
    ...
    with tf.variable_scope(name):
        ...
        def mean_var_train():
            ...
            with tf.control_dependencies([test_apply_op]):
                return (tf.identity(alpha * batch_mean + (1 - alpha) * test_ema.average(batch_mean)), 
                        tf.identity(alpha * batch_var + (1 - alpha) * test_ema.average(batch_var)))
...
def misconception_layer(inputs, filter_count, is_training, filter_size=3, 
                        decay=None, padding="SAME", name='misconception_layer'):
    ...
    with tf.variable_scope(name):
        ...
        #
        pool = tf.nn.max_pool(inputs, [1, filter_size, 1, 1], [1, 1, 1, 1], 
                                padding=padding, data_format='NHWC') 
        #
        joint = leaky_rectify(
                    batch_norm(
                        tf.concat(3, [conv, pool]),
                        is_training, **decay_arg))
        #
        return leaky_rectify(
                    batch_norm(
                        conv1d_layer(joint, 1, filter_count, name="NIN"),
                        is_training, **decay_arg))
