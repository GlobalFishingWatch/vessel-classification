...
class Model(ModelBase):
    ...
    def build_training_file_list(self, base_feature_path, split):
        ...
        training_ids = self.vessel_metadata.fishing_range_only_list(random_state, split)
        return [
    ...
    def make_input_fn(self, base_feature_path, split, parallelism, prefetch):
        def input_fn():
            return (
                fishing_feature_generation.input_fn(
    ...
    def make_training_input_fn(
        self, base_feature_path, num_parallel_reads, prefetch=1024
    ):
        return self.make_input_fn(
            base_feature_path, metadata.TRAINING_SPLIT, num_parallel_reads, prefetch
        )

    def make_test_input_fn(self, base_feature_path, num_parallel_reads, prefetch=1024):
        return self.make_input_fn(
            base_feature_path, metadata.TEST_SPLIT, num_parallel_reads, prefetch
        )
