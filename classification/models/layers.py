...
def misconception_layer(
    inputs, filters, kernel_size, strides, training, scope=None, virtual_batch_size=None
):
    ...
    with tf.name_scope(scope):
        ...
        stage_conv = ly.batch_normalization(
            stage_conv, training=training, virtual_batch_size=virtual_batch_size
        )
        ...
        total = ly.batch_normalization(
            total, training=training, virtual_batch_size=virtual_batch_size
        )
        total = tf.nn.relu(total)
        return total


def misconception_with_bypass(
    inputs, filters, kernel_size, strides, training, scope=None, virtual_batch_size=None
):
    with tf.name_scope(scope):
        residual = misconception_layer(
            inputs, filters, kernel_size, strides, training, scope, virtual_batch_size
        )
        if strides > 1:
...
def misconception_fishing(
    ...
    for _ in range(post_layers - 1):
        embedding = ly.conv1d(
            embedding, post_filters, 1, activation=None, use_bias=False
        )
