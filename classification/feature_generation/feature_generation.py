...
def parse_function_core(example_proto, num_features):
    context_features, sequence_features = tf.io.parse_single_sequence_example(
        example_proto,
        context_features={"id": tf.io.FixedLenFeature([], tf.int64)},
        sequence_features={
            "movement_features": tf.io.FixedLenSequenceFeature(
...
def read_input_fn_infinite(
    paths, num_features, num_parallel_reads=4, random_state=None, weights=None
):
    ...
    return tf.data.TFRecordDataset(path_ds, num_parallel_reads=num_parallel_reads).map(
        parse_function, num_parallel_calls=num_parallel_reads
    )
