...
def input_fn(
    ...
    def add_labels(features, timestamps, time_bounds, id_):
        ...
        labels = tf.py_func(
            _add_labels, [id_, timestamps], [tf.float32] * len(objectives)
        )
...
def predict_input_fn(
    paths, num_features, time_ranges, window_size, min_timeslice_size, parallelism=4
):
    random_state = np.random.RandomState()
    ...
    def xform(id_, movement_features):
        ...
        features, timestamps, time_ranges_tensor, id_ = tf.py_func(
            _xform, [id_, raw_features], [tf.float32, tf.int32, tf.int32, tf.string]
        )
