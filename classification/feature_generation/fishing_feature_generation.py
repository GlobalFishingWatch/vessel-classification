...
def input_fn(
    ...
    def xform(id_, movement_features):
        def _xform(id_, features):
            ...
            return feature_utilities.extract_n_random_fixed_points(
                random_state, features, num_slices_per_id, window_size, id_, ranges
            )
    ...
    def add_labels(features, timestamps, time_bounds, id_):
        ...
        [labels] = tf.compat.v1.py_func(_add_labels, [id_, timestamps], [tf.float32])
    ...
    return (
        raw_data.map(xform, num_parallel_calls=parallelism)
...
def predict_input_fn(
    paths, num_features, window_size, start_date, end_date, window, parallelism=4
):
    if window is None:
        b, e = 0, window_size
    else:
    ...
    raw_data = feature_generation.read_input_fn_one_shot(
        paths, num_features, num_parallel_reads=parallelism
    )
