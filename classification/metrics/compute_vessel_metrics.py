...
import sys
import yattag
from classification.metadata import (
...
class InferenceResults(object):
    ...
    def __init__(
        self,  # TODO: Consider reordering args so that label_list is first
        ids,
        inferred_labels,
        true_labels,
        start_dates,
        ...
        label_list,
        all_ids=None,
        all_inferred_labels=None,
        all_true_labels=None,
        all_start_dates=None,
        all_scores=None,
    ):
...
LocalisationResults = namedtuple(
    "LocalisationResults", ["true_fishing_by_id", "pred_fishing_by_id", "label_map"]
)
...
def precision_score(y_true, y_pred, sample_weights=None):
    ...
    true_pos = np.array(
        [(x == y and x != 0) for (x, y) in zip(y_true, y_pred)], dtype=float
    )
...
def ydump_attrs(doc, results):
    ...
    for dt in np.unique(results.start_dates):
        ...
        rows.append(
            [
                dt,
                RMS(results.true_attrs[mask], results.inferred_attrs[mask]),
    ...
    with tag("div", klass="unbreakable"):
        ...
        ydump_table(
            doc,
            ["Start Date", "RMS Error", "Abs Error"],
    ...
    with tag("div", klass="unbreakable"):
        ...
        text(
            "{:.2f}".format(
                RMS(
                    consolidated.true_attrs[true_mask & infer_mask],
...
def ydump_metrics(doc, results, weights_map):
    ...
    rows = [
        (
            x,
            accuracy_score(
                results.true_labels, results.inferred_labels, (results.start_dates == x)
            ),
    ...
    with tag("div", klass="unbreakable"):
        ...
        weights = composite_weights(
            weights_map, results.mapping, consolidated.true_labels
        )
...
def precision_recall_f1(labels, y_true, y_pred, weights):
    ...
    for lbl in labels:
        ...
        if trues.sum() and positives.sum():
            ...
            results.append(
                (
                    lbl,
                    precision_score(trues, positives),
    ...
    results.append(
        (
            "ALL (by prevalence)",
            precision_score(y_true, y_pred, sample_weights=weights),
...
def consolidate_across_dates(results, date_range=None):
    ...
    return InferenceResults(
        np.array(inferred_ids),
        np.array(inferred_labels),
...
def consolidate_attribute_across_dates(results):
    ...
    return AttributeResults(
        ids,
        np.array(inferred_attributes),
        np.array(true_attributes),
        np.array(true_labels),
        None,
    )
...
class ClassificationExtractor(InferenceResults):
    ...
    def finalize(self):
        ...
        for lbl in self.label_list:
            ...
            logging.info(
                "%s true and %s inferred labels for %s", true_count, inf_count, lbl
            )
...
def assemble_composite(results, mapping):
    ...
    return InferenceResults(
        trim(results.all_ids),
        trim(inferred_labels),
        trim(true_labels),
        trim(start_dates),
        trim(inferred_scores),
        label_list,
        np.array(results.all_ids),
        np.array(inferred_labels),
        np.array(true_labels),
...
def compute_results(args):
    ...
    label_df = pd.read_gbq(
        "select * from `{}`".format(args.label_table),
    ...
    for row in label_df.itertuples():
        ...
        for field in [
            ...
            if (
                ...
                if field == "label":
                    if row.label.strip() not in VESSEL_CLASS_DETAILED_NAMES:
    ...
    for field in ["length", "tonnage", "engine_power", "crew_size"]:
        for id_, value in maps[field].items():
            if float(value) <= 0:
                logging.warning("%s has a values of %s for %s", id_, value, field)
    ...
    ext = AttributeExtractor("crew_size", maps["crew_size"], maps["label"])
    ...
    for field in ["length", "tonnage", "engine_power", "crew_size"]:
        if not all(results[field].inferred_attrs >= 0):
            logging.warning(
                ...
                field,
                min(results[field].inferred_attrs),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dest-path", required=True, help="output path to write results to"
    )
    parser.add_argument(
        "--T", default=1.0, type=float, help="Temperature to adjust scores by"
    )
