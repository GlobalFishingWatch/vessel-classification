...
FishingRange = namedtuple("FishingRange", ["is_fishing", "start_time", "end_time"])
...
def ydump_fishing_localisation(doc, results):
    ...
    rows = []
    logging.info("Overall localisation accuracy %s", accuracy_score(y_true, y_pred))
    logging.info("Overall localisation precision %s", precision_score(y_true, y_pred))
    logging.info("Overall localisation recall %s", recall_score(y_true, y_pred))
    ...
    for cls in sorted(set(fishing_category_map.values())) + ["other"]:
        ...
        if len(true_chunks):
            ...
            rows.append(
                [
                    "{} ({}:{}/{})".format(
                        cls, len(true_chunks), sum(y_true), len(y_true)
                    ),
...
def load_true_fishing_ranges_by_id(
    fishing_range_path, split_map, split, threshold=True
):
    ...
    with open(fishing_range_path) as f:
        for row in csv.DictReader(f):
            ...
            rng = (
                val,
                parse(row["start_time"]).replace(tzinfo=pytz.UTC),
...
def compare_fishing_localisation(
    inferred_ranges, fishing_range_path, label_map, split_map, split
):
    logging.debug("loading fishing ranges")
    true_ranges_by_id = load_true_fishing_ranges_by_id(
        fishing_range_path, split_map, split
    )
...
def compute_results(args):
    ...
    results["localisation"] = compare_fishing_localisation(
        fishing_ranges, args.fishing_ranges, maps["label"], maps["split"], args.split
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--project-id", help="Google Cloud project id", default="world-fishing-827"
    )
    parser.add_argument("--label-path", help="path to test data", required=True)
    parser.add_argument(
        "--fishing-ranges", help="path to fishing range data", required=True
    )
    parser.add_argument("--dest-path", help="path to write results to", required=True)
