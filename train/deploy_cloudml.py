...
def launch(args):
    ...
    job_id = (
        ("%s_%s_%s" % (args.model_name, args.job_name, timestamp))
        .replace(".", "_")
        .replace("-", "_")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", required=True, help="environment for run: prod/dev.")
    parser.add_argument("--model_name", required=True, help="module name of model.")
    parser.add_argument("--job_name", required=True, help="unique name for this job.")
    parser.add_argument("--feature_path", required=True, help="gcs path to features.")
    parser.add_argument(
        "--vessel_info", required=True, help="local path to vessel_info."
    )
    parser.add_argument(
        "--fishing_ranges", default="", help="optional local path fishing ranges"
    )
    parser.add_argument(
        "--config_file", default="deploy_cloudml.yaml", help="configuration file path."
    )
    parser.add_argument(
        "--split", default=0, type=int, help="Split to use (-1) for all"
    )
